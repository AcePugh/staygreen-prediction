...
def get_hyperparameters():
    try:
        learning_rate = simpledialog.askfloat("Input", "Enter Learning Rate (e.g., 0.1):", minvalue=0.01, maxvalue=1.0)
        if learning_rate is None:
            return None
        max_depth = simpledialog.askinteger("Input", "Enter Max Depth (e.g., 6):", minvalue=1)
        if max_depth is None:
            return None
        min_child_weight = simpledialog.askinteger("Input", "Enter Minimum Child Weight (e.g., 1):", minvalue=1)
        if min_child_weight is None:
            return None
    except Exception as e:
        messagebox.showerror("Input Error", f"An error occurred: {e}")
        return None
    return learning_rate, max_depth, min_child_weight
...
def main():
    ...
    model = xgb.XGBRegressor(learning_rate=learning_rate, max_depth=max_depth, min_child_weight=min_child_weight, random_state=random_state)
